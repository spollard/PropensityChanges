...
sub MultithreadCommands {
	...
	#print "$number_of_worker_threads worker threads will be used\n";
	...
	for my $thread (@$threads) {
		#print "Waiting for thread " . $thread->tid() . " to finish\n";
		$thread->join();
	}
	...
	#print "You can continue with analysis knowing that all commands are complete\n";
}
...
sub worker_thread {
	...
	# none left
	#print "Starting thread " . threads->tid() . "\n"; 
	while (my $command = $command_queue->dequeue_nb()) {
		#print "Running '$command' on thread " . threads->tid() . "\n";
	...
	#print "Thread " . threads->tid() . " finished processing commands\n"
}